...
// Upvote/Remove Upvote Event
export const upvoteEvent = async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        const hasUpvoted = event.upvoters.includes(user_id);
        
        if (hasUpvoted) {
            // Remove upvote
            event.upvoters = event.upvoters.filter(
            ...
            return res.status(200).json({ 
                ...
                upvotes: event.upvoters.length,
                upvoters: event.upvoters,
                hasUpvoted: false
            });
        }else {
            // Add upvote
            event.upvoters.push(user_id);
            await event.save();
